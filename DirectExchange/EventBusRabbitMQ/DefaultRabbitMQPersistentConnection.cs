...
namespace EventBusRabbitMQ
{
    public class DefaultRabbitMQPersistentConnection : IRabbitMQPersistentConnection
    {
        ...
        public DefaultRabbitMQPersistentConnection(IConnectionFactory connectionFactory,
           Serilog.ILogger logger, int retryCount = 5)
        {
        ...
        public IModel CreateModel()
        {
            if (!IsConnected)
            {
                _logger.Error("No RabbitMQ connections are available to perform this action");
        ...
        public void Dispose()
        {
            ...
            {
                _logger.Error($"Dispose erorr {ex.Message}");
                throw;
            }
        }
        ...
        public bool TryConnect()
        {
            lock (sync_root)
            {
                var policy = RetryPolicy.Handle<SocketException>()
                  ...
                  {
                      _logger.Error("RabbitMQ Client could not connect after {TimeOut}s ({ExceptionMessage})", $"{time.TotalSeconds:n1}", ex.Message);
                  }
              );
                policy.Execute(() =>
                {
        ...
        private void OnCallbackException(object sender, CallbackExceptionEventArgs e)
        {
            ...
            _logger.Error("A RabbitMQ connection throw exception. Trying to re-connect...");
